        ...
        while steps < N_STEPS:
            ...
            if (steps % SAVING_INTERVAL == 0 and steps > 0) or steps >= N_STEPS:
                agent.policy_net.save(
                    f"agents/dqn-resnet/models/{timestamp}/{steps}.pth"
                )
                agent.draw_model(
                    f"agents/dqn-resnet/plots/training/{timestamp}/{steps}.png"
                )
                agent.draw_agent(
                    f"agents/dqn-resnet/plots/threshold/{timestamp}/{steps}.png"
                )
                draw(
                    envs,
                    save_path=f"agents/dqn-resnet/plots/env/{timestamp}/{steps}.png",
                )
...
def saving(
    ...
    os.makedirs(f"agents/dqn-resnet/histories/{timestamp}/env/", exist_ok=True)
    os.makedirs(f"agents/dqn-resnet/histories/{timestamp}/training/", exist_ok=True)
    os.makedirs(f"agents/dqn-resnet/histories/{timestamp}/threshold/", exist_ok=True)

    eps_hist_df.to_csv(
        f"agents/dqn-resnet/histories/{timestamp}/env/{steps}.csv",
        index=False,
    )
    stats_df.to_csv(
        f"agents/dqn-resnet/histories/{timestamp}/training/{steps}.csv", index=False
    )
    thres_df.to_csv(
        f"agents/dqn-resnet/histories/{timestamp}/threshold/{steps}.csv", index=False
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        envs.close()
        tracer.stop()
        tracer.save(f"agents/dqn-resnet/tracings/{timestamp}.json")
